...
//    val expectedUser: User = UserBuilder(defaultCompanyId = Some(expectedCompany.id)).build
...
//    when(companyRepository.findById(expectedUser.defaultCompanyId.get)).thenReturn(Future.successful(Some(expectedCompany)))
//
